...
async function tuyaRequest(token: string, method: 'GET', path: string) {
    const t = Date.now().toString();
    const nonce = crypto.randomUUID();
    const bodyString = '';
...
async function checkSupportedEndpoints() {
    try {
        ...
        console.log(`Checking supported categories for IR Hub: ${irId}`);

        // Get supported categories
        const res = await tuyaRequest(token, 'GET', `/v2.0/infrareds/${irId}/categories`);
