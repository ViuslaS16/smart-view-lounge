...
async function testProjectorEndpoint() {
    try {
        const token = await getAccessToken();
        const irId = process.env.TUYA_IR_DEVICE_ID;
        const remoteId = process.env.TUYA_PROJECTOR_REMOTE_ID;

        console.log(`Testing Projector Endpoint: /v2.0/infrareds/${irId}/projectors/${remoteId}/command`);

        // Test PowerOn
        const res = await tuyaRequest(token, 'POST', `/v2.0/infrareds/${irId}/projectors/${remoteId}/command`, {
            code: "PowerOn"
        });
        console.log('Response for PowerOn:', JSON.stringify(res, null, 2));
