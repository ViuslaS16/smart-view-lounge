...
async function bruteForcePower() {
    try {
        ...
        const variations = ["PowerOn", "power_on", "poweron", "power", "Power", "on", "Power On", "power on"];

        for (const v of variations) {
            console.log(`Testing variation: "${v}"`);
            const res = await tuyaRequest(token, 'POST', endpoint, { code: v });
